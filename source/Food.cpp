...
Food GenerateFood(const SnakeGame* game, int boardWidth, int boardHeight)
{
    ...
    for (const SnakeNode* node = game->head; node != nullptr; node = node->next)
    {
        ++occupiedCount;
    }
...
bool EatFood(Food* food, SnakeGame* game, int boardWidth, int boardHeight)
{
    ...
    if (game->head == nullptr || game->head->x != food->x || game->head->y != food->y)
    {
        return false;
    }
